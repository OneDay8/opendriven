...
'''
def opendriven_cost(begin,interval,df_list,df_data):
    data = df_list.copy()
    df_data['flag'] = 0 # 持仓标记，持多仓：1，不持仓：0，持空仓：-1
    record_buy = []
    record_sell = []
    ...
    stats = evaluation(pd_buy,pd_sell, df_data) #计算评价指标
    # Output into Excel
    with pd.ExcelWriter("开盘动量_cost.xlsx") as writer:
        df_data.to_excel(writer, sheet_name='stock_data')
        pd_transactions.to_excel(writer, sheet_name='trade_record')
        stats.to_excel(writer, sheet_name='evaluations')
        ...
        if min(data[day]['low'][0:interval]) <= min(data[day]['low'][interval:interval*2]) and min(data[day]['low'][interval:interval*2]) <= min(data[day]['low'][interval*2:interval*3]) \
            ...
            for i in range(int(len(data[day]['high'])/interval-5)):
                ...
                >= max(data[day]['high'][interval*(i+4):interval*(i+5)]):
                                ...
                                price_stop_1 = np.average(data['close'][interval*(i+5):interval*(i+5)+5], weights=data['volume'][interval*(i+5):interval*(i+5)+5])*(1-0.00013)
                                ...
                                if (maxhigh - data[day]['low'].iloc[interval*(i+5)-1]) >= ATR*2.5:
                                    ...
                                    price_stop_2 = np.average(data['close'][interval*(i+5):interval*(i+5)+5], weights=data['volume'][interval*(i+5):interval*(i+5)+5])*(1-0.00013)
            ...
            price_stop_0=np.average(data['close'][-5:-1], weights=data['volume'][-5:-1])*(1-0.00013) 
            ...
            for i in range(int(len(data[day]['high'])/interval-3)):
                ...
                if min(data[day]['low'][interval*i:interval*(i+1)]) <= min(data[day]['low'][interval*(i+1):interval*(i+2)]) \
                    and min(data[day]['low'][interval*(i+1):interval*(i+2)]) <= min(data[day]['low'][interval*(i+2):interval*(i+3)]):
                        price_stop.append(data[day]['open'].iloc[interval*(i+3)])
                        return_after_section.append((price_stop[-1]-price_stop[-2])*((3-n)/3)/price_stop[-2])
                        n = n+1
                ...
                if (data[day]['high'].iloc[interval*(i+3)-1] - minlow) >= TR*2:
                    price_stop.append(data[day]['open'].iloc[interval*(i+3)])
                    return_after_section.append((price_stop[-1] - price_stop[-2])*((3-n)/3)/price_stop[-2])
                    n = n+1
                if n == 3:
...
def opendriven_stop_night(begin,interval,df_list,df_data):
    ...
    for day in range(len(data)-1):
        ...
        if min(data[day]['low'][0:interval]) <= min(data[day]['low'][interval:interval*2]) and min(data[day]['low'][interval:interval*2]) <= min(data[day]['low'][interval*2:interval*3]) \
            ...
            for i in range(int(len(data[day]['high'])/interval-5)):
                ...
                #反向止损
                if max(data[day]['high'][interval*i:interval*(i+1)]) >= max(data[day]['high'][interval*(i+1):interval*(i+2)]) \
                    and max(data[day]['high'][interval*(i+1):interval*(i+2)]) >= max(data[day]['high'][interval*(i+2):interval*(i+3)]) \
                        and max(data[day]['high'][interval*(i+2):interval*(i+3)]) >= max(data[day]['high'][interval*(i+3):interval*(i+4)]) \
                            and max(data[day]['high'][interval*(i+3):interval*(i+4)]) >= max(data[day]['high'][interval*(i+4):interval*(i+5)]):
                                
                                price_stop.append(data[day]['open'].iloc[interval*(i+5)])
                                return_after_section.append((price_stop[-1] - price_stop[-2])*((3-n)/3)/price_stop[-2])
                                n = n+1
                if n == 3:
                    break
                ...
                TR = max(high_low,preclose_high,preclose_low)
                maxhigh = max(data[day]['high'][0:interval*(i+5)])
                if (maxhigh - data[day]['low'].iloc[interval*(i+5)-1]) >= TR*2.5:
                    price_stop.append(data[day]['open'].iloc[interval*(i+5)])
                    return_after_section.append((price_stop[-1] - price_stop[-2])*((3-n)/3)/price_stop[-2])
                    n = n+1
                if n == 3:
    ...
    #data['simple_return'] = data.close.pct_change(1).fillna(0)
    df_data['nav'] = (1+df_data.return_after_all.fillna(0)*df_data.flag-df_data.return_after_cost.fillna(0)).cumprod()
    df_data['benchmark'] = df_data.close / df_data.close[0] #将CLOSE[0]的净值设定为1，计算基准净值
